Ext.define('web.view.SfsClientController', {
	...
	history: function(msg) {
		this.lookupReference('history').getStore().insert(0, {type:'log',reqcmdId: "",rescmdId: "", content:msg});
	},
	clearRequest: function() {
		console.log("clear");
		var store = this.lookupReference('requestParams').getStore();
		store.removeAll();
		store.sync();
	}, 
	historyClick: function( e, record) {
		var data = record.data.request;
		
		var req = this.getViewModel().set("cmd", record.data.reqcmdId)
		...
		this.getViewModel().set("content", new JSONFormat(record.data.content, 4));
	},
	historyDoubleClick: function( e, record) {
		var data = record.data.request;
		var req = this.getViewModel().set("cmd", record.data.reqcmdId)
	...
	onConnection: function(event)
	{
		if (event.success)
		{
			this.history({"msg":"Connected to SmartFoxServer 2X!<br>","SFS2X API VERSION" : this.sfs.version});
	...
	onConnectionLost: function(event)
	{
		console.log(event);
		this.history(event);
        this.lookupReference('history').getStore().insert(0, {type:'connection lost',reqcmdId: "",rescmdId: "", content:event.reason});
	...
	onExtensionResponse: function(event)
	{
		...
		var req = this.getViewModel().get("request")
        this.lookupReference('history').getStore().insert(0, {type:'response',reqcmdId: this.getViewModel().get("cmd"),rescmdId: event.cmd, request: req, content:message});
		this.getViewModel().set("content", new JSONFormat(message, 4).toString());
	},
	onLoginError: function(evtParams)
	{
        this.lookupReference('history').getStore().insert(0, {type:'error',reqcmdId: '',rescmdId: event.cmd, content:event.params.getUtfString("infor")});
	},
	onDebugLogged: function(event)
	{
		this.history(event.message);
	},
	...
	{
		// Log message
		this.history({"msg":"Disconnecting..."});
	...
	{
		var store = this.lookupReference('requestParams').getStore();
		store.insert(0, {property:'userId',value: this.getViewModel().get("uid")});
		store.insert(0, {property:'serverId',value: this.getViewModel().get("serverId")});
	},
	...
	send: function(){
		...
		this.getViewModel().set("request",{});
		var modelRequest = this.getViewModel().get("request");
		var store = this.lookupReference('requestParams').getStore().each(
			function(x){
				modelRequest[x.data.property] = x.data.value;
				
			}
		);
		
	
		var requestContent = JSON.stringify(modelRequest);
		
		console.log(requestContent);
		var param = new SFS2X.SFSObject();
		param.putInt("index", this.index++);
		param.putUtfString("infor",requestContent);
		var req = new SFS2X.ExtensionRequest(cmd,param);
		this.sfs.send(req);
	},
	addParamField(e, eopts) {
		if (eopts.keyCode==13) {
			console.log(e, eopts);
		var store = this.lookupReference('requestParams').getStore().insert(0,{});
		}
	},
